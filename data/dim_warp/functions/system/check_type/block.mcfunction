...
# 空白ブロックのチェック
execute if entity @s[tag=Success] if block ^ ^ ^ air if block ^ ^1 ^ air if block ^ ^2 ^ air if block ^ ^2 ^-1 air if block ^-1 ^1 ^ air if block ^-1 ^ ^ air run tag @s add Gate_Block_Check
